...
export default function ProjectPage() {
	...
	const { data: environments, isLoading: envsLoading, isError: envsError } = useEnvironments(slug);
	const [activeEnv, setActiveEnv] = useState<string>("");
	const selectedEnv = activeEnv || environments?.[0]?.slug || "";

	const { data: flags, isLoading: flagsLoading, isError: flagsError } = useFlags(slug, selectedEnv);
	...
	async function handleCreateEnv(e: React.FormEvent) {
		e.preventDefault();
		try {
			const envSlug = envName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
				...
				>
					<form onSubmit={handleCreateEnv} className="space-y-4">
						...
						<div className="flex justify-end gap-3 pt-2">
							<Button type="button" variant="outline" onClick={() => setCreateEnvOpen(false)}>
								{tc("cancel")}
							</Button>
